...
class TariffRead(BaseModel):
    ...
    resource_type: str = Field(description="Тип ресурса: xvs — холодная вода (ХВС), gvs — горячая вода (ГВС), electric — электричество, rent — аренда/наём")
...
class TariffUpdate(BaseModel):
    ...
    resource_type: str = Field(description="Тип ресурса: xvs, gvs, electric, rent")
...
class ReadingCreate(BaseModel):
    ...
    xvs_value: float = Field(ge=0, description="Показание счётчика холодной воды (ХВС)")
    gvs_value: float = Field(ge=0, description="Показание счётчика горячей воды (ГВС)")
...
class ReadingRead(BaseModel):
    ...
    xvs_value: float = Field(description="Показание счётчика холодной воды (ХВС)")
    gvs_value: float = Field(description="Показание счётчика горячей воды (ГВС)")
...
class MonthlyCostRead(BaseModel):
    ...
    xvs_consumption: float = Field(description="Объём потребления холодной воды (м³)")
    gvs_consumption: float = Field(description="Объём потребления горячей воды (м³)")
    electric_consumption: float = Field(description="Объём потребления электроэнергии (кВт·ч)")
    xvs_cost: float = Field(description="Стоимость холодной воды за месяц")
    gvs_cost: float = Field(description="Стоимость горячей воды за месяц")
...
class YearSummary(BaseModel):
    ...
    total_xvs_cost: float = Field(description="Суммарная стоимость холодной воды за год")
    total_gvs_cost: float = Field(description="Суммарная стоимость горячей воды за год")
